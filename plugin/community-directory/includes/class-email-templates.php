...
class CD_Email_Templates {
    ...
    /**
     * Get a custom template override from wp_options (if any).
     *
     * @param string $key Template key (e.g. 'invite', 'rejection').
     * @return array|null Array with 'subject' and 'body' keys, or null if no override.
     */
    private static function get_custom( $key ) {
        $all = get_option( 'cd_email_templates', array() );
        if ( ! empty( $all[ $key ] ) && ( ! empty( $all[ $key ]['subject'] ) || ! empty( $all[ $key ]['body'] ) ) ) {
            return $all[ $key ];
        }
        return null;
    }
    ...
    public static function send_invite( $email, $first_name, $token ) {
        ...
        $custom = self::get_custom( 'invite' );
        if ( $custom ) {
            $subject = ! empty( $custom['subject'] ) ? self::render( $custom['subject'], $vars ) : __( 'Welcome to St. Thekla Community Directory — Set Up Your Account', 'community-directory' );
            $message = ! empty( $custom['body'] ) ? self::render( $custom['body'], $vars ) : null;
        }

        if ( empty( $message ) ) {
            $subject = $subject ?? __( 'Welcome to St. Thekla Community Directory — Set Up Your Account', 'community-directory' );
            $message = sprintf(
                __(
                    "Hello %1\$s,\n\n" .
    ...
    public static function send_rejection( $email, $first_name, $rejection_reason, $notes = '' ) {
        ...
        $vars = array( 'first_name' => esc_html( $first_name ), 'reason' => $reason_text );
        $custom  = self::get_custom( 'rejection' );
        $subject = __( 'St. Thekla Community Directory — Application Update', 'community-directory' );

        if ( $custom && ! empty( $custom['body'] ) ) {
            if ( ! empty( $custom['subject'] ) ) { $subject = self::render( $custom['subject'], $vars ); }
            $message = self::render( $custom['body'], $vars );
        } else {
            $message = sprintf(
                __(
                    "Hello %1\$s,\n\n" .
    ...
    public static function send_info_request( $email, $first_name, $message_text ) {
        $vars = array( 'first_name' => esc_html( $first_name ), 'message' => esc_html( $message_text ) );
        $custom  = self::get_custom( 'info_request' );
    ...
    public static function send_officer_added( $email, $first_name, $title = '' ) {
        $title_line = '';
        if ( ! empty( $title ) ) {
            $title_line = sprintf( __( "Role: %s\n", 'community-directory' ), esc_html( $title ) );
        }

        $vars = array( 'first_name' => esc_html( $first_name ), 'title_line' => $title_line );
        $custom  = self::get_custom( 'officer_added' );
    ...
    public static function send_officer_removed( $email, $first_name ) {
        $vars = array( 'first_name' => esc_html( $first_name ) );
        $custom  = self::get_custom( 'officer_removed' );
    ...
    public static function send_password_reset( $email, $name, $token ) {
        ...
        $vars = array( 'display_name' => esc_html( $name ), 'reset_url' => esc_url( $reset_url ) );
        $custom  = self::get_custom( 'password_reset' );
    ...
    public static function send_email_hint( $email, $first_name, $masked ) {
        ...
        $vars = array( 'first_name' => esc_html( $first_name ), 'masked_email' => $masked, 'login_url' => esc_url( $login_url ) );
        $custom  = self::get_custom( 'email_hint' );
    ...
    public static function send_household_invite( $email, $first_name, $token, $inviter = null ) {
        ...
        $custom  = self::get_custom( 'household_invite' );
    ...
    public static function notify_officers_of_approval( $member_data, $approved_by ) {
        ...
        $custom = self::get_custom( 'officer_notification' );
        $subject = sprintf(
